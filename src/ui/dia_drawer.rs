...
pub fn draw_train(viewer: &DiagramViewer, ui: &Ui, diagram_data: &DiaFile, train: &Train) -> Vec<Shape> {
	...
	let station_y = get_station_y(diagram_data, viewer.display_line_index).iter().map(|v| v * viewer.scale_y).collect::<Vec<f32>>();
	...
	for i in 0..train.station_times.len()-1 {
		...
		if before_sta.arrive.is_some() && before_sta.departure.is_some() {
			shapes.extend(Shape::dashed_line(&[
				Pos2 {
					x: (before_sta.arrive.unwrap().get_total_minute() as f32) * x_scale + x + DiagramViewer::OFFSET_X + start_pos.x, 
					y: station_y[before_sta.station_index] + y + DiagramViewer::OFFSET_Y + start_pos.y}, 
				Pos2 {
					x: (before_sta.departure.unwrap().get_total_minute() as f32) * x_scale + x + DiagramViewer::OFFSET_X + start_pos.x, 
					y: station_y[before_sta.station_index] + y + DiagramViewer::OFFSET_Y + start_pos.y}], 
					train_type.0, train_type.1, train_type.2));
		}
		shapes.extend(Shape::dashed_line(&[
			Pos2 {
				x: (time.1.unwrap().get_total_minute() as f32) * x_scale + x + DiagramViewer::OFFSET_X + start_pos.x, 
				y: station_y[time.0] + y + DiagramViewer::OFFSET_Y + start_pos.y}, 
			Pos2 {
				x: (time.3.unwrap().get_total_minute() as f32) * x_scale + x + DiagramViewer::OFFSET_X + start_pos.x, 
...
fn traintype_to_stroke(diagram_data: &DiaFile, train: &Train) -> (Stroke, f32, f32) {
	...
	match train_type.line_type {
		LineType::Solid => {
			(Stroke { width: DEFAULT_WIDTH, color: arr_to_color32(&train_type.line_color) }, 100.0, 0.0)
		},
		LineType::Bold(width) => {
			(Stroke { width, color: arr_to_color32(&train_type.line_color) }, 100.0, 0.0)
		},
		LineType::Dashed(width, dash_length, gap_length) => {
			(Stroke { width, color: arr_to_color32(&train_type.line_color) }, dash_length, gap_length)
		},
	}
}
...
pub fn draw_time_line(viewer: &DiagramViewer, ui: &Ui, diagram_data: &DiaFile) -> Vec<Shape> {
	...
	for i in (0..=24*60).step_by(2) {
		let line = eframe::epaint::Shape::dashed_line(&[
			Pos2 {x: (i as f32) * x_scale + DiagramViewer::OFFSET_X + x + start_pos.x, y: DiagramViewer::OFFSET_Y + y + start_pos.y, },
			Pos2 {x: (i as f32) * x_scale + DiagramViewer::OFFSET_X + x + start_pos.x, y: *station_y.last().unwrap() + DiagramViewer::OFFSET_Y + y + start_pos.y, },
	...
	for i in (0..=24*60).step_by(10) {
		...
		shapes.push(Shape::vline(
			...
			Stroke {width:0.6, color:Color32::GRAY},));
	}
	for i in (0..=24*60).step_by(30) {
		shapes.push(Shape::vline(
			...
			std::ops::RangeInclusive::new(
				DiagramViewer::OFFSET_Y + y + start_pos.y, 
				*station_y.last().unwrap() + DiagramViewer::OFFSET_Y + y + start_pos.y), 
			Stroke {width:2.0, color:Color32::GRAY},));
	}
